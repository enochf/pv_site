...
{
	if ($method) {
		return Thirty360USdayCount($start_date, $end_date);
	} else {
		return Thirty360EUdayCount($start_date, $end_date);
	}
}
...
{
	return Thirty360USdayCount($startdate, $enddate) / 360.0;
}
...
{
	return Thirty360EUdayCount($startdate, $enddate) / 360.0;
}
...
function ActualActualyearFraction($startdate, $enddate, $refPeriodStart = null, $refPeriodEnd = null)
{
	...
	/*
	if (DATEDIFF('day', $startdate, $enddate) == 0) return 0.0;
	$d1 = getdate($startdate);
	$d2 = getdate($enddate);
	...
	$sum += ActualActualdayCount($startdate, mktime(0,0,0,1,1,$d1['year'] + 1)) / $dib1;
	$sum += ActualActualdayCount(mktime(0,0,0,1,1,$d2['year']), $enddate) / $dib2;
	return $sum;
	*/
	...
	$months = intval(0.5 + 12 * DATEDIFF('day', $refPeriodStart, $refPeriodEnd) / 365);
	$period = $months / 12.0;
	...
	if ($enddate <= $refPeriodEnd) {
		...
		if ($startdate >= $refPeriodStart) {
			...
			return $period * ActualActualdayCount($startdate, $enddate) /
				ActualActualdayCount($refPeriodStart, $refPeriodEnd);
		} else {
			...
			$previousRef = DATEADD('month', -$months, $refPeriodStart);
			if ($enddate > $refPeriodStart)
				return ActualActualyearFraction($startdate, $refPeriodStart, $previousRef,
							$refPeriodStart) +
						ActualActualyearFraction($refPeriodStart, $enddate, $refPeriodStart,
							$refPeriodEnd);
			else
				return ActualActualyearFraction($startdate,$enddate,$previousRef,$refPeriodStart);
		}
	} else {
		...
		$sum = ActualActualyearFraction($startdate, $refPeriodEnd, $refPeriodStart, $refPeriodEnd);
		...
		$i = 0;
		do {
			$newRefStart = DATEADD('month', $months * $i, $refPeriodEnd);
			$newRefEnd   = DATEADD('month', $months * ($i + 1), $refPeriodEnd);
		...
		$sum += ActualActualyearFraction($newRefStart, $newRefEnd, $newRefStart, $newRefEnd);
		return $sum;
	}
}
...
{
	return Actual360dayCount($startdate, $enddate) / 360.0;
}
...
{
	return Actual365dayCount($startdate, $enddate) / 365.0;
}
...
function VDB($cost, $salvage, $life, $start_period, $end_period, $factor = 2.0, $no_switch = false)
{
	...
	if ($no_switch) {
		...
		for ($i = $nLoopStart + 1; $i <= $nLoopEnd; $i++) {
			$fTerm = _ScGetGDA($cost, $salvage, $life, $i, $factor);
			if ($i == $nLoopStart + 1)
...
{
	$ppmt = ($pv * 1.0) / ($nper * 1.0);
	for($i=0; $i <= $per; ++$i) {
		$value = $rate * $pv * -1;
		$pv -= $ppmt;
		if($i == $nper) { // principle must be 0 after the final payment
			$value = 0;
		}
	}
	return($value);
...
{
	if (!_is_valid_basis($basis)) return null;
	if (!_is_valid_frequency($frequency)) return null;
...
function COUPNUM($settlement, $maturity, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360)
{
	...
	switch ($basis) {
		case FINANCIAL_BASIS_MSRB_30_360: // US(NASD) 30/360
			$dsm = Thirty360USdayCount($settlement, $maturity);
			break;
		case FINANCIAL_BASIS_ACT_ACT: // Actual/actual
			$dsm = ActualActualdayCount($settlement, $maturity);
			break;
		case FINANCIAL_BASIS_ACT_360: // Actual/360
			$dsm = Actual360dayCount($settlement, $maturity);
			break;
		case FINANCIAL_BASIS_ACT_365: // Actual/365
			$dsm = Actual365yearFraction($settlement, $maturity);
			break;
		case FINANCIAL_BASIS_30E_360: // European 30/360
			$dsm = Thirty360EUdayCount($settlement, $maturity);
			break;
	}
...
function COUPDAYBS($settlement, $maturity, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360)
{
	...
	switch ($frequency) {
		...
		case 2: // semiannual
			return 365 - ($dsm % 360);
		case 4: // quarterly
			return DATEDIFF('day', DATEADD('day', -ceil($dsm / 90) * 90 - ($dsm % 90), $maturity), $settlement);
	}
	return null;
}
?>